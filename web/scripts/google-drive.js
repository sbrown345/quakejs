...
function initStuff() {
    retrieveAllFiles("mimeType='application/vnd.google-apps.folder' and title='quake_js' and trashed = false", function (files) {
        ...
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            if (file && file.title == "quake_js") {
                $("body").append("<div><img src=\"" + file.iconLink + "\">" +
                    "<a href=\"" + "#" + "\">" + file.title + "</a></div>");

                var fileToUpload = new Uint8Array(10);
                fileToUpload[0] = 72;
                fileToUpload[1] = 73;
                insertFileFromBase64("test.txt", file.id, base64ArrayBuffer(fileToUpload.buffer), function () {
